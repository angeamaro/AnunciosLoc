...
import ao.co.isptec.aplm.anunciosloc.ui.view.LocalActivity;
...
public class LocationsFragment extends Fragment implements OnMapReadyCallback {
    ...
    private void addLocationsToMap(List<Location> locations) {
        ...
        for (Location location : locations) {
            if (location.getLatitude() == 0 && location.getLongitude() == 0) continue;
            ...
            if (marker != null) marker.setTag(location);

            CircleOptions circleOptions = new CircleOptions()
                    .center(position)
                    .radius(location.getRadiusInMeters())
                    .strokeColor(0x880078D4)
                    .fillColor(0x220078D4)
                    .strokeWidth(2);
    ...
    private void setupListeners() {
        ...
        if (btnMenu != null) {
            btnMenu.setOnClickListener(v -> {
                Intent intent = new Intent(requireContext(), ao.co.isptec.aplm.anunciosloc.ui.view.MenuOptionsActivity.class);
                startActivity(intent);
            });
        }

        // Clique no contador para abrir LocalActivity
        if (txtLocationCount != null) {
            txtLocationCount.setOnClickListener(v -> {
                Intent intent = new Intent(requireContext(), LocalActivity.class);
                startActivity(intent);
            });
        }
    }
